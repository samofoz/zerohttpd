    ...
    while (1) {
        ...
        switch (req->event_type) {
            case EVENT_TYPE_ACCEPT:
                ...
                add_accept_request(server_socket, &client_addr, &client_addr_len);
                connections++;
                if(max_concurrent < connections){
                    max_concurrent = connections;
                    //fprintf(stderr, "New connection! max_concurrent = %d\n", max_concurrent);
                }
                //fprintf(stderr, "New connection! %d [%d]\n", connections, server_port);
            ...
            case EVENT_TYPE_READ:
                if (!cqe->res) {
                    ...
                    close(req->client_socket);
                    --connections;
                    //fprintf(stderr, "Closed connection! %d [%d]\n", connections, server_port);
                    free(req);
                    break;
                }
                //fprintf(stderr, "EVENT_TYPE_READ!\n");
                handle_client_request(req);
                free(req->iov[0].iov_base);
                free(req);
                break;
            case EVENT_TYPE_WRITE:
                //fprintf(stderr, "EVENT_TYPE_WRITE!\n");
                for (int i = 0; i < req->iovec_count-1; i++) {
                    free(req->iov[i].iov_base);
                }
                close(req->client_socket);
                gettimeofday(&(req->stop), NULL);
                //fprintf(stderr, "Request took %lu us\n", (req->stop.tv_sec - req->start.tv_sec) * 1000000 + req->stop.tv_usec - req->start.tv_usec);
                --connections;
                //fprintf(stderr, "Closed connection! %d [%d]\n", connections, server_port);
                free(req);
                break;
